...
public class ToBeRefactored {
  ...
  public static void main(String[] args) {
    ...
    while (cash > 0) {
      ...
      if (hasBlackJack(handvalue) && dealer
          .hasBlackJack())//check if both the user and dealer have blackjack.
      {
        Push();
    ...
    System.out.println("Your cash is: "
        + cash);//if user doesn't want to play or runs out of cash, either congratulates them on their winnings or lets them know
    if (cash == 0) {
  ...
  public static int calcHandValue(List<Card> hand) {
    ...
    int handvalue = 0;
    handvalue = Dealer.getHandvalue(aHand, handvalue);
    return handvalue;
  }
